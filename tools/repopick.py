...
parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
    ...
    patches from SOKP Gerrit instance.
...
def fetch_query(query):
    url = 'http://review.sonic-developers.com/changes/?q=%s' % query
    if args.verbose:
...
for changeps in args.change_number:
    ...
    #   )]}'
    #   [ ... valid JSON ... ]
    url = 'http://review.sonic-developers.com/changes/?q={change}&o={query_revision}&o=CURRENT_COMMIT&pp=0'.format(change=change, query_revision=query_revision)
    if args.verbose:
